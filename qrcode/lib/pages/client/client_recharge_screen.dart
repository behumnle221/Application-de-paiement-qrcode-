...
class ClientRechargeScreen extends StatefulWidget {
  const ClientRechargeScreen({super.key});

  @override
  State<ClientRechargeScreen> createState() => _ClientRechargeScreenState();
}

class _ClientRechargeScreenState extends State<ClientRechargeScreen> {
  ...
  Future<void> _recharger() async {
    ...
    try {
      final result = await ClientService.rechargerCompte(
        montant: montant,
      ...
      if (result['success'] == true) {
        if (mounted) {
          _showSuccessDialog(result['message'] ?? 'Rechargement initiated!');
        }
      } else {
        setState(() {
          _errorMessage = result['message'] ?? 'Erreur lors du rechargement';
        });
      }
    } catch (e) {
      setState(() {
                      ...
                      Container(
                        ...
                        child: const Row(
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                'Rechargez votre compte virtuel depuis votre compte Mobile Money (Orange ou MTN)',
                                style: TextStyle(
                                  fontSize: 14,
